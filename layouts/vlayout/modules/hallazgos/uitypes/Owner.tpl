...
{if $FIELD_MODEL->get('uitype') eq '53'}
	...
	{/if}
	{if $FIELD_MODEL->isReadOnly() eq true}
		<input type="hidden" value="{$FIELD_VALUE}" name="{$ASSIGNED_USER_ID}"/>
		{foreach key=OWNER_ID item=OWNER_NAME from=$ALL_ACTIVEUSER_LIST}
			{if $FIELD_VALUE eq $OWNER_ID}
				{assign var=LABEL_VALUE value=$OWNER_NAME}
			{/if}
		{/foreach}
		{foreach key=OWNER_ID item=OWNER_NAME from=$ALL_ACTIVEGROUP_LIST}
			...
			{/if}
		{/foreach}
		<input id="{$MODULE}_editView_fieldName_{$FIELD_NAME}" type="text" class="input-large {if $FIELD_MODEL->isNameField()}nameField{/if}" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true}required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" name="label_{$FIELD_MODEL->getFieldName()}" value="{$LABEL_VALUE}"
